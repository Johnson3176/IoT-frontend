...
class Maku {
    ...
    scene; // 所属场景
    segments; // 细分数
    constructor(el, material, scene, config = {}) {
        const { meshType = "mesh", meshSizeType = "size", segments = {
            width: 64,
            height: 64,
        }, textureUniform = "uTexture", useTextureLoader = true, } = config;
        this.el = el;
        this.scene = scene;
        ...
        const texture = useTextureLoader && !(el instanceof HTMLCanvasElement)
        ...
        const geometryMap = {
            size: new THREE.PlaneGeometry(width, height, segments.width, segments.height),
